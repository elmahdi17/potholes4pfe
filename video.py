...
cap = cv.VideoCapture("video route.mp4")
    ...
    for (classid, score, box) in zip(classes, scores, boxes):
        ...
        if len(scores) != 0 and scores[0] >= 0.7:
            if (recarea / area) <= 0.1 and box[1] < 600:
                ...
                # Ensure the resized dimensions are valid
                if new_width > 0 and new_height > 0:
                    ...
                    # Ensure the text_position coordinates are within the frame dimensions
                    if (0 <= text_position[1] < frame.shape[0] - new_height) and (0 <= text_position[0] < frame.shape[1] - new_width):
